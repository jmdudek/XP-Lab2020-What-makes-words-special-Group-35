...
```{r, fig.align=, fig.height=9, fig.width=9}
# creating data set for our first hypothesis
data_hypothesis1 <- data_agg %>% 
  #filter(image_category != "dog") %>% 
  # calculating the mean RT for each cue type (label, sound), to get a first intuition of the differences it RT
...
hypo1_re <- brm(
  RT_log ~ cue_type + (1 || subject_id) + (cue_type || category),
  # data
  data = data_hypothesis1,
  # seed
  seed = 1
...
1-pnorm(-loo_comp1[2,1], loo_comp1[2,3])
```
...
1-pnorm(-loo_comp2[2,1], loo_comp2[2,3])
```
...
1-pnorm(-loo_comp3[2,1], loo_comp3[2,3])
```
...
1-pnorm(-loo_comp4[2,1], loo_comp4[2,3])
``` 
